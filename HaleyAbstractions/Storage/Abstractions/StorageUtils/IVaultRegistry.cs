...
namespace Haley.Abstractions {
    public interface IVaultRegistry {
        ...
        Task<IFeedback> RegisterClient(IVaultProfileControlled client, string password = null);
        Task<IFeedback> RegisterModule(IVaultProfileControlled module, IVaultProfileControlled client); //If a client is not registered, we register it against "Default"
        Task<IFeedback> RegisterWorkSpace(IVaultProfileControlled wspace, IVaultProfileControlled client,IVaultProfileControlled module, VaultControlMode content_control = VaultControlMode.Number, VaultParseMode content_pmode = VaultParseMode.Generate); //If a client is not registered, we register it against "Default"
